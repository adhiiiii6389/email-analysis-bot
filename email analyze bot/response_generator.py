...
class ResponseGenerator:
    ...
    def batch_generate_responses(self, analyzed_emails: List[Dict]) -> List[Dict]:
        """
        Generate responses for multiple analyzed emails in batch.
        Returns a list of emails with generated responses.
        """
        complete_emails = []
        total_emails = len(analyzed_emails)
        
        print(f"🤖 Starting batch response generation for {total_emails} emails...")
        
        for i, email_data in enumerate(analyzed_emails, 1):
            try:
                print(f"📝 Generating response {i}/{total_emails}: {email_data.get('subject', 'No Subject')[:50]}...")
                
                # Get analysis data
                analysis_data = email_data.get('analysis', {})
                
                # Generate response
                response_data = self.generate_response(email_data, analysis_data)
                
                # Combine email data with response
                complete_email = {
                    **email_data,  # Original email data + analysis
                    'response': response_data  # Generated response
                }
                
                complete_emails.append(complete_email)
                
            except Exception as e:
                logger.error(f"Error generating response for email {i}: {e}")
                # Add email with error status
                error_response = {
                    'response_text': f"Sorry, we encountered an error generating a response for this email. Please contact support manually. Error: {str(e)}",
                    'response_type': 'error',
                    'priority': analysis_data.get('priority', 'normal'),
                    'category': analysis_data.get('category', 'General Support'),
                    'generated_at': datetime.now().isoformat(),
        ...
        print(f"✅ Batch response generation completed. {len(complete_emails)} responses generated.")
